...
delete_intervals(CorpusList *cp, Bitfield intervals, int mode)
{
  ...
  if (modified) {
    ...
    if (modified == cp->size) {
       ...
       * Erhaltung der Sortierung einfaellt
       *
...
apply_range_set_operation(CorpusList *corpus1, RangeSetOp operation, CorpusList *corpus2, Bitfield restrictor)
{
  ...
  default:
    ...
    assert(0 && "Illegal operator in RangeSetOp");
    return 0;
    break;
 ...
 *
 * @return    Boolean: true = success, false = one of various kinds of failure.
 */
static int
SortExternally(void)
{
    ...
    if (text_size > 0) {
      for (line = 0; line < srt_cl->size; line++) {
        ...
        default:
          ...
          return 0; /* to reassure the compiler */
        }
