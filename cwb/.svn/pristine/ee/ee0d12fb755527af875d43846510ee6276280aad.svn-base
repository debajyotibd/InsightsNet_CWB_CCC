 ...
 *  - TO DO: implement a suitable Sort operation and revise Uniq, or implement a join SortUniq operation, possibly as a separate function
 ...
 * many operations expect that the matchlist(s) is/are consistently sorted, but don't specify the expected sort order
 *
...
apply_setop_to_matchlist(Matchlist *list1, MLSetOp operation, Matchlist *list2)
{
  ...
  case Complement:
    ...
    if (tmp.tabsize == 0) {
      ...
      /*
       * something in between the best and worse cases.
       */
      tmp.start = (int *)cl_malloc(sizeof(int) * tmp.tabsize);
      tmp.end = NULL;
      tmp.target_positions = NULL;
