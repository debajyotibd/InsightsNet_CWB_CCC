...
main(int argc, char **argv) {
  ...
  if (optind < argc) {
    ...
    /* now that we have the corpus, we can check the encoding of the rx, if one was specified. */
    if ( rx && !cl_string_validate_encoding(rx, cl_corpus_charset(corpus), 0) ) {
      fprintf(stderr, "Cannot use the specified regular expression, as it is not compatible with the corpus character encoding.\n");
      exit(1);
    }
