...
/* above also brings in  (on *nix) sys/resource.h ; (on Win) winsock2.h and windows.h */
 ...
 *
 * NOTE: this history-writing is not dependent on readline being
 * available / enabled - but auto-reloading of the history on startup
 * DOES only happen if use_readline is true..
 *
 * @see QueryBuffer
...
addHistoryLine(void)
{
  ...
  if (cqp_history_file != NULL    &&
      ...
      !reading_cqprc) {
    if (QueryBuffer[0] != '\0') {
      /* history file must be a real file but we want to allow auto-magic expansion of ~ */
      if (!(dst = cl_open_stream(cqp_history_file, CL_STREAM_APPEND, CL_STREAM_MAGIC_NOPIPE)))
...
do_XMLTag(char *s_name, int is_closing, int op, char *regex, int flags)
{
  ...
  if (generate_code) {
    ...
    if (!is_closing && regex) {
      ...
      if (flags == IGNORE_REGEX || ((strcspn(regex, CL_REGEX_METACHARACTERS) == strlen(regex)) && (flags == 0) && (op_type == OP_EQUAL))) {
        ...
        default:
          ...
          return 0; /* reassure compiler that pattern will never bne uninitialised */
        }
...
do_flagged_string(char *s, int flags)
{
  ...
  if (generate_code) {
    ...
    if ( IGNORE_REGEX == flags || (strcspn(s, CL_REGEX_METACHARACTERS) == strlen(s) && 0 == flags) ) {
