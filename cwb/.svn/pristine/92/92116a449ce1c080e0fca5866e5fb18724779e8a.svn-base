...
latex_print_context(ContextDescriptor *cd, FILE *dest)
{
  ...
  switch(cd->left_type) {
  case char_context:
    fprintf(dest, "%d characters", cd->left_width);
    break;
  case word_context:
    fprintf(dest, "%d tokens", cd->left_width);
    break;
  case s_att_context:
    fprintf(dest, "%d %s", cd->left_width, cd->left_structure_name ? cd->left_structure_name : "???");
    break;
  default:
    break;
  }
  ...
  case char_context:
    fprintf(dest, "%d characters", cd->right_width);
    break;
  case word_context:
    fprintf(dest, "%d tokens", cd->right_width);
    break;
  case s_att_context:
    fprintf(dest, "%d %s", cd->right_width, cd->right_structure_name ? cd->right_structure_name : "???");
    break;
  default:
    break;
  }
...
latex_print_group(Group *group, FILE *dest)
{
  ...
  fprintf(dest, "\\begin{tabular}{llr}\n");
