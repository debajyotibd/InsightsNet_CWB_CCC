 ...
 *
 * <a href="http://cwb.sourceforge.io">http://cwb.sourceforge.io</a>
 *
 */
...
/** enable automagic, but fail on attempt to open pipe (safe mode for filenames from external sources); DOES allow stdin / stdout */
...
/** read/write xz-compressed file */
#define CL_STREAM_XZ           7
...
/**
 * This variable can be set to True to make CL streams use 7z instead of gzip/bzip2 to handle compressed files.
 * 7z may be available on operating systems where gzip/bzip2 are absent or hard to get hold of.
 * On library startup, this variable is set to true if the environment variable CWB_USE_7Z was set;
 * otherwise, to false.
 */
extern int cl_stream_compress_with_7z;
...
/**
 * Set of regular expression meta-characters. If a search string contains none of these characters
 * and no flags are set (IGNORE_CASE | IGNORE_DIAC), it can safely be looked up as a literal string.
 * Npte that this is not a complete list of "unsafe" characters that may have special meaning in particular contexts.
 */
#define CL_REGEX_METACHARACTERS "[](){}.*+|?\\^$"
