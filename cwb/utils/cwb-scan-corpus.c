...
/* an old TO-DO, probably will never actually be done now:
   rewrite hash entries so that K-tuples can be embedded in the HashEntry struct,
...
scancorpus_add_key(char *key)
{
  ...
  if (!cl_string_validate_encoding(key, cl_corpus_charset(C), 0)) {
    fprintf(stderr, "Cannot process key '%s': character encoding conflicts with that of corpus.\n", key);
    exit(2);
  }
  if (Hash.N >= MAX_N) {
    fprintf(stderr, "Error: too many key specifiers (at most %d keys are allowed)\n", MAX_N);
    exit(1);
  }
