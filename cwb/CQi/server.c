...
#else
      1 != send(connfd, (char *)&prep, 1, MSG_WAITALL)
#endif
      )  {
...
#else
  if (bytes != recv(connfd, (char *)buf, bytes, MSG_WAITALL)) {    /* windows recv() has 2nd arg char * instead of void * */
#endif
...
#else
  if (1 != recv(connfd, (char *)&b, 1, MSG_WAITALL)) {
#endif
