...
cl_read_stream(PositionStream ps, int *buffer, int buffer_size)
{
  ...
  if (ps->is_compressed) {
    ...
    /* Some versions of GCC 11 seem to check for memcpy() possibly exceeding 2^63-1 bytes (though, why bother?
       any realistic limit would be much smaller); signed items_to_read means there could possibly be a negative
       result here, which would wrap around to >= 2^63 in the size_t argument to memcpy(). So GCC complains.
       The (unsigned) cast on the line below silences the warning (because now GCC's inference works out). */
    memcpy(buffer, ps->base + ps->nr_items, ((unsigned)items_to_read) * sizeof(int));
 ...
 * is within a region on the given s-attribute
 * (i.e. a structure, an instance of that s-attribute);
 * and/or on a boundary; or outside a region.
 *
 * @see STRUC_INSIDE
 * @see STRUC_LBOUND
 * @see STRUC_RBOUND
