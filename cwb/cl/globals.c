...
#include <locale.h>
#ifdef _WIN64
   /* definitions and includes needed for the Windows "built-in" registry to work in cl_startup(). */
#  include <ctype.h>
#  include <libgen.h>
#  include <libloaderapi.h>
   /* build environment should have provided these, these are just fallbacks in case not. */
#  ifndef DEFAULT_REGISTRY_LOCFILE
#    define DEFAULT_REGISTRY_LOCFILE "default-reg.txt"
#  endif
#  ifndef DEFAULT_REGISTRY_FALLBACK
#    define DEFAULT_REGISTRY_FALLBACK "%ProgramFiles%\\CorpusWorkbench\\Registry"
#  endif
#endif
...
#ifdef _WIN64
/**
 * Contains the path of the default registry on Windows (where it can't safely be set at compile time)
 */
char cl_variable_registry[CL_MAX_FILENAME_LENGTH];
#endif
 ...
 * doing anything else.
 *
 * Currently, it does two things. First, it makes sure that string case-insensitive comparison
 * will work the same everywhere regardless of the locale set by the system or user.
 *
 * Second, it looks for the environment variable CWB_USE_7Z and sets the
 * corresponding internal variable appropriately.
 */
void
cl_startup(void)
{
  char *cp_tmp; /* compressor path - temp store */
  int len;
  ...
  cl_stream_compress_with_7z = (NULL != getenv(CWB_USE_7Z_ENVVAR));

  cp_tmp = getenv(CWB_COMPRESSOR_PATH_ENVVAR);
  if (cp_tmp) {
    cl_strcpy(cl_compressor_path, cp_tmp);
    len = strlen(cl_compressor_path);
    if ( ! ('/' == cl_compressor_path[len-1] || '\\' == cl_compressor_path[len-1]) ) {
      if (len+1 < CL_MAX_FILENAME_LENGTH) {
        cl_compressor_path[len] = SUBDIR_SEPARATOR;
        cl_compressor_path[len+1] = '\0';
      }
    }
    /* else do nothing, it already ends in a slash. */
  }

#ifdef _WIN64
  /* handling of an install-time default registry. Cf. definitions in cwb-globals.h */
  {
    char exe[CL_MAX_FILENAME_LENGTH];
    GetModuleFileNameA(NULL, exe, CL_MAX_FILENAME_LENGTH);
    if (exe[0]) {
      FILE *src;
      char fn[CL_MAX_FILENAME_LENGTH];

      sprintf(fn, "%s\\%s", basename(exe), DEFAULT_REGISTRY_LOCFILE);
      if (NULL != (src = fopen(fn, "r"))) {
        size_t l;
        fgets(cl_variable_registry, CL_MAX_FILENAME_LENGTH, src);
        fclose(src);
        for(l = strlen(cl_variable_registry) ; l >= 0 && isspace(cl_variable_registry[l]) ; l --)
          cl_variable_registry[l] = '\0';
      }
    }
    /* this kicks in if (a) exe path not obtained, (2) couldn't open file, (3) file is just full of whitespace. */
    if (!cl_variable_registry[0])
      cl_strcpy(cl_variable_registry, DEFAULT_REGISTRY_FALLBACK);
  }
#endif
}
