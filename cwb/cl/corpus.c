 ...
 *          CWB_REGISTRY_ENVVAR or, failing that, the macro
 *          CWB_REGISTRY_DEFAULT_PATH.
 * @see     CWB_REGISTRY_ENVVAR
 * @see     CWB_REGISTRY_DEFAULT_PATH
 */
char *
...
#ifdef _WIN64
  static char buf[260] = { '\0' }; /* maxlen of directory var content in Win64 is 259 */
#endif
...
#ifdef _WIN64
  /* we  check here for the %ProgramFiles% / %HOMEDRIVE%%HOMEPATH% /  %UserProfile% variables, and resolve them if present.
   * first: check for an env variable in first position. */
  if (regdir && '%' == regdir[0]) {
    /* Now we know there is something to check. We need case-insensitive  comparison for Windows. */
    if (0 == strncasecmp(regdir, "%ProgramFiles%", 14))
      sprintf(buf, "%s%s", getenv("PROGRAMFILES"), regdir+14);
    else if (0 == strncasecmp(regdir, "%HomeDrive%%HomePath%", 21))
      sprintf(buf, "%s%s%s", getenv("HOMEDRIVE"), getenv("HOMEPATH"), regdir+21);
    else if (0 == strncasecmp(regdir, "%UserProfile%", 13))
      sprintf(buf, "%s%s", getenv("USERPROFILE"), regdir+13);
    /* no need to worry about resolving them anywhere except at the beginning. */
  }
  if (*buf)
    regdir = buf;
  /* regdir remains as it was if nothing was printed to buf. */
#endif
