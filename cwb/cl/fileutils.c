 ...
 * to do some configuration twiddling to get that to work; it's not 100%
 * clear). Even so, however, it's not clear whether stat() in MinGW will
 * correctly report what is and is not a symlink.
 *
 * See https://blogs.windows.com/windowsdeveloper/2016/12/02/symlinks-windows-10/
 *
 * @param path  Path to check.
 * @return      Boolean. (False if file is not link or if these was an error.)
 */
int
is_link(char *path)
{
...
/*
 * Boolean flag for whether or not to use the 7z tool rather than gzip/bzip2/xz.
 * (This is made public in cl.h, and initialised in globals.c)
 */
int cl_stream_compress_with_7z = 0;

/**
 * Holds the compression binaries path ('' by default; initialised in globals.c;
 * exported within CL, but not elsewhere). It is set up so that, if it contains
 * a path, then there will definitely be a separator '/' on the end.
 */
char cl_compressor_path[CL_MAX_FILENAME_LENGTH] = { '\0' };
...
#define STREAM_IS_PIPE(type) (type == CL_STREAM_PIPE || type == CL_STREAM_GZIP || type == CL_STREAM_BZIP2 || type == CL_STREAM_XZ)
...
{
  char *point, *mode_spec, *prog = NULL;
  ...
  if (type == CL_STREAM_MAGIC || type == CL_STREAM_MAGIC_NOPIPE) {
    /* expand ~/ or $HOME/  (or %userprofile%)
       (for clarity, some parts are repeated in both the Win and *nix branches here) */
#ifndef _WIN64
...
#else
    int shift = 0;
    /* test the envvar FIRST, to avoid the complex tests on the filename if we can't get it. */
    char *home = getenv("USERPROFILE");
    if (home && home[0] != '\0') {
      if ('~' == filename[0] && ('/' == filename[1] || '\\' == filename[1]))
        shift = 2;
      else if (0 == strncasecmp(filename, "$home", 5) && ('/' == filename[5] || '\\' == filename[5]))
        /* we might as well continue to accept $HOME...  also, don't use the separator constants, because we want to allow both */
        shift = 6;
      else if (0 == strncasecmp(filename, "%userprofile%", 13) && ('/' == filename[13] || '\\' == filename[13]))
        shift = 14;
      if (shift) {
        snprintf(expanded_filename, 2 * CL_MAX_FILENAME_LENGTH, "%s\\%s", home, filename + shift);
        filename = expanded_filename;
        l = strlen(filename); /* don't forget to update string length */
      }
    }
    ...
    if (cl_str_is(filename, "-"))
      ...
      else if (l > 4 && 0 == strcasecmp(filename + l - 4, ".xz"))
        type = CL_STREAM_XZ;
    }
  }
  ...
  if (CL_STREAM_GZIP == type || CL_STREAM_BZIP2 == type || CL_STREAM_XZ == type) {
    if (!(handle = fopen(filename, mode_spec))) {
      cl_errno = CDA_EPOSIX;
      return NULL;
    }
    fclose(handle);
  ...
  case CL_STREAM_XZ:
    ...
    /* case any compressed type... they all have the same patterns with standalone or with 7z */
    point = g_shell_quote(filename);
    if (mode == CL_STREAM_APPEND) {
      sprintf(command, cl_stream_compress_with_7z ? "%s7z a dummy -t%s -si -so >> %s" : "%s%s >> %s", cl_compressor_path, prog, point);
      mode_spec = (mode_spec[1] == 'b' ? "wb" : "w");
    }
    else if (mode == CL_STREAM_WRITE)
      sprintf(command, cl_stream_compress_with_7z ? "%s7z a dummy -t%s -si -so > %s"  : "%s%s > %s", cl_compressor_path, prog, point);
    else
      sprintf(command, cl_stream_compress_with_7z ? "%s7z e -t%s -so %s"              : "%s%s -cd %s", cl_compressor_path, prog, point);
  ...
  /* add to start of list of managed streams */
...
/* if Win64 *is* defined we'll get a compiler warning for "was_pipe" as an unused variable.  This warning can be ignored. */
#endif
