 ...
 *
 * HISTORICAL NOTE "Set the appropriate values to the corpus id (given by its pointer to
 * the symbol table)." --> was the original comment for this function, but it does not seem
 * to relate to this function at all. Preserved for posterity.
 *
...
eval_constraint(AVS avs, int corppos, RefTab labelrefs, RefTab target_labelrefs, int *out_cpos)
{
  ...
  case Anchor:
    ...
    default:
      ...
      return False;  /* not reached, but let's reassure the compiler that this is a dead-end */
    }
