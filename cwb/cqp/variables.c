...
/* An old TO-DO, probably not worth doing anyway: "shrink malloced buffers as necessary - for VarSpace as well
 ...
 *
 * @param varname   Variable to act on.
...
SetVariableValue(const char *varname, char operator, char *values)
{
  ...
  case '<':                        /* < operator: overwrite by loading values from file */
    VariableDeleteItems(v);
...
/**
 * Get array of lexicon IDs of those of the Variable's strings that are found in the lexicon of the given corpus/attribute.
 *
 ...
 * @param nr_items  Out parameter: will be set to the number of integers in the returned array
 *                  (likewise, to 0 if the return value is NULL).
 ...
 *                  Caller has responsibiltiy to free this block as it is newly allocated.
 */
int *
