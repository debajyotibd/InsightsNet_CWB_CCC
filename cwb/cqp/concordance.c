...
/* the following code is borrowed from <utils/cwb-decode.c> and ensures that XML tags in the kwic output always nest properly */
...
# define add_sep_to_line_if_not_empty(l, s) {if (l->len > 0) cl_autostring_concat(l, s);}
  ...
  case char_context:
    ...
    length_characters = 0; /* we only need to keep track of the character count here */
    enough_context = 0;
    ...
    for (start = match_end + 1; (start < text_size && !enough_context); start++) {
      ...
      if (length_characters >= cd->right_width)
        ...
        compose_kwic_token(cd, start, token, pdr);
        if (line->len > 0) {
          /* Including the token separator between match and co-text into the character count for RightContext
           * is counter-intuitive (because "set RightContext 1;" shows no context at all) and inconsistent
           * (because the separator before the match is not included in the count for LeftContext).
           * So we need to special-case the first separator (start == match_end + 1) to exclude it from the count.
           */
          cl_autostring_concat(line, tok_sep);
          if (start > match_end + 1)
            length_characters += cl_charset_strlen(corpus->charset, tok_sep);
        }
        /* else: don't add token separator initially in line (shouldn't happen in right context anyway) */
        ...
        if (length_characters > cd->right_width) {
          if (corpus->charset != utf8) {
            ...
            length_characters = cd->right_width;
          }
          else {
