...
void StateQueue_clear(StateQueue self)
{
  ...
  while (self->head) {
    this = self->head;
    self->head = this->next;
    FSAState_delete(&this); /* an old TO DO: move state to free list instead of deallocating */
  }
  self->n_states = 0;
}
...
int StateQueue_pop(StateQueue self, RefTab output_rt)
{
  ...
  FSAState_delete(&state);          /* An old TO DO: move state to free list instead, in order to avoid frequent double malloc() calls */
  self->n_states--;
  return cpos;
}
