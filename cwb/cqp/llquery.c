...
/**
 * Reload the contents of the CQP history file, if it exists.
 *
 * We have to roll our own, rather than use built in readline/read_history(),
 * because the history file was created with CL_STREAM magic;
 * therefore it must be read with CL_STREAM magic or else "~/"
 * won't be handled correctly.
 */
static void
reload_cqp_history(void)
{
  if (cqp_history_file && '\0' != cqp_history_file[0]) {
    FILE *src;
    if (!(src = cl_open_stream(cqp_history_file, CL_STREAM_READ, CL_STREAM_MAGIC_NOPIPE)))
      cqpmessage(Error, "Can't open history file %s\n", cqp_history_file);
    else {
      char line[CL_MAX_LINE_LENGTH];
      char *mark;
      int len;
      while (fgets(line, CL_MAX_LINE_LENGTH, src)) {
        /* trim off the trailing "\n" (and/or "\r"), and any leading spaces. */
        mark = line;
        len = strlen(line);
        ...
        if (0 != len)
          add_history(mark);
        /* We can overwrite, cos readline's add_history()
           allocates its own storage for everything passed in. */
      }
      cl_close_stream(src);
    }
  }
