...
/* Windows, and some dodgy old Unixes like Solaris, don't have setenv(), so we need to emulate it with putenv() */
...
pt_get_anchor_cpos(CorpusList *cl, int n, FieldType anchor, int offset)
{
  ...
  default:
    assert(0 && "Can't be");
    return 0; /* not reached, but let's reassure the compiler that cpos cna't be unset */
  }
