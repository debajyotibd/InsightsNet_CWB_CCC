...
duplicate_corpus(CorpusList *cl, char *new_name, Boolean force_overwrite)
{
  ...
  if (newc) {
    ...
    /* does copying the query_* fields really make sense? */
    ...
    /* An old, never done TO-DO: decide whether we should copy the position_lists */
  }
 ...
 * it is overwritten.
 *
 * Note that this is a replica of LOTS AND LOTS of the code from duplicate_corpus.
 *
      ...
      while ((ep = readdir(dp))) {
        if (!strchr(ep->d_name, '.') &&   /* ignore files with '.' char in registry (such as hidden files) */
          ...
          if (ct == SUB) {
            ...
            /* The function check_stamp used to be used here to check the format of saved-query files.
...
attach_subcorpus(CorpusList *cl, char *advertised_directory, char *advertised_filename)
{
  ...
  if ( cl && (cl->type == SUB || cl->type == TEMP) ) {
    ...
    if (!src)
      ...
      if (8 > (len = file_length(fullname)))
        fprintf(stderr, "ERROR: File length of subcorpus is < 8 bytes\n");
      else {
          ...
          if ( ! (mother && mother->corpus) )
...
#if subcorpload_debug
            {
              ...
              fprintf(stderr,
                      ...
                      "regdir: %s\n"
                      "regname: %s\n",
                      (long int)(p - field), /* typecast from ptrdiff_t (%td) */
                      cl->size,
                      cl->registry,
                      cl->mother_name);
              for (j = 0; j < cl->size; j++)
...
access_corpus(CorpusList *cl)
{
  if (cl) {
    if (cl->loaded) {
      ...
      case SYSTEM:
        assert(0);  /* because a system corpus is never a saved SC/NQR. So this is a logic error. */
        return True;
